...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['delete'])) {
        ...
        $deleteQuery = $mysqli->prepare("UPDATE grades
        ...
        if ($deleteQuery->execute()) {
            ...
            header("Location: course_grade.php?course_code=$courseCode");
            exit();
        } else {
            echo "Grade deletion failed: " . $mysqli->error;
        }
        ...
        if ($updateQuery->execute()) {
            // Successful update, refresh the page
            header("Location: course_grade.php?course_code=$courseCode");
            exit();
        } else {
        ...
        <?php foreach ($courseGradeData as $row) : ?>
            <tr>
                ...
                <td>
                    <form method="POST">
                        <input type="hidden" name="student_email" value="<?= $row['student_epita_email'] ?>">
                        <button type="submit" name="delete" class="update-button">Delete</button>
                    </form>
                </td>
            </tr>
        <?php endforeach; ?>
